	...
	$.ajax({
		...
		success : function(data) {
			...
			$.each(data.list, function(i, pd) {
				// 登陆者资料
				$("#user_info")
						.html('<small>Welcome</small> ' + pd.NAME + '');
				if(pd.depId != null){
					$("#dep_info").html(pd.depName);
				}else{
					$("#dep_info").remove();
				}
				user = pd.USERNAME;
				USER_ID = pd.USER_ID;// 用户ID
				hf(list.SKIN);// 皮肤
				...
				if (pd.PASSWORD == pd.defaultPsd) {
					jzts();
					var diag = new top.Dialog();
					diag.Drag = true;
					diag.Title = "修改密码";
					diag.URL = locat + '/head/toEditPsd.do';
					diag.Width = 680;
					diag.Height = 500;
					diag.CancelEvent = function() { // 关闭事件
						var flag = diag.innerFrame.contentWindow.document
								.getElementById('issuccess').value;
						if (flag != 1) {
							bootbox.alert("尚未完成修改密码所有步骤，请完成所有步骤后点击完成按钮，感谢配合。")
						} else {
							diag.close();
						}
						// diag.close();
...
function online() {
	if (window.WebSocket) {
		...
		websocket.onmessage = function(message) {
			if(message != null && message.data.indexOf('[join]') < 0){
