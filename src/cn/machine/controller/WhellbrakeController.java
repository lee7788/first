...
public class WhellbrakeController {
@Resource
  private WheelbrakeService wheelbrakeService;
/**
 * 分页 -- rows/total
 * 
 * @param pageSize
...
public Map<String, Object> listPage(Integer pageSize, Integer offset) {
	...
	try {
		List<Car>cars = wheelbrakeService.listPage(pageSize, offset);
		map.put("rows", cars);
		List<Car> listAll =wheelbrakeService.listAll();
...
public  ModelAndView showaddmodel(Integer tId){
	...
	List<Car> men=wheelbrakeService.serchid(tId);
	mView.addObject("men",men);
	mView.setViewName("houtai/center/menu/Wheelbrake_update");
	return mView;
	}
/**
 *  更新按钮实现方法
...
Map<String,Object> map=new HashMap<String,Object>();	
wheelbrakeService.typeupdate(car);
map.put("msg", "success");
return map;
	   }	
}
